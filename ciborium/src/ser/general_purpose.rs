...
/// Serializes a value as CBOR into a type implementing [`ciborium_io::Write`]
#[inline]
...
pub fn into_vec<T: ?Sized + ser::Serialize>(
    value: &T,
) -> Result<alloc::vec::Vec<u8>, Error<<alloc::vec::Vec<u8> as ciborium_io::Write>::Error>> {
    let mut vector = alloc::vec![];
